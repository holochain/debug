...
async fn main() {
    ...
    let alice_arc = DhtArc::new(alice.get_loc(), ideal_length);

    conductor
        .add_agent_infos(
            agent_info
                .iter()
                .filter(|a| alice_arc.contains(a.storage_arc.center_loc()))
                .cloned()
                .collect(),
        )
        .await
    ...
    while let Some((msg, respond)) = channel.next().await {
        ...
        match msg.msg {
            ...
            HolochainP2pMockMsg::PeerGet(kwire::PeerGet { agent: request, .. }) => {
                let info = gen_data.peer_get(&to_agent, request).unwrap();
                respond
                    .unwrap()
