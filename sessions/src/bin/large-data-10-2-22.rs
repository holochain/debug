...
async fn main() {
    ...
    for _ in 0..NUM_CONDUCTORS {
        ...
        let handle =
            SweetConductor::handle_from_existing(envs.path(), test_keystore(), &config, &[]).await;
        let conductor = SweetConductor::new(handle, envs.into_tempdir(), config.clone()).await;
    ...
    for c in conductors.iter() {
        if let Some(db) = &existing_db {
            let root_path = c.db_path().to_path_buf();
    ...
    for (i, c) in cells.iter().enumerate().skip(1) {
        for (j, h) in headers.iter().enumerate() {
            let mut cascade = Cascade::empty().with_dht(c.dht_db().clone().into());
