    ...
    while let Some((msg, respond)) = channel.next().await {
        ...
        match msg.msg {
            ...
            HolochainP2pMockMsg::PeerGet(kwire::PeerGet { agent: request, .. }) => {
                let info = gen_data.peer_get(&to_agent, request);
                match info {
                    Some(info) => {
...
async fn dump_data(
    ...
    env.async_commit(move |txn| {
        for (khash, (_, element)) in gen_data.agent_khash_idx.values().flatten() {
...
async fn run_locals(
    ...
    let ideal_length =
        mock_network::agent_info::ideal_target(num_agents + num_locals, DhtArc::full(0)).coverage();
...
async fn run_remotes<'session>(
    ...
    {
        let ideal_length =
            mock_network::agent_info::ideal_target(num_agents + 1, DhtArc::full(0)).coverage();
        let ils = ideal_length.to_string();
        let bus = boot_url.to_string();
        ...
        let mut out = session
            .run(
                ...
                // ],
                // &["RUST_LOG=warn,[get]=debug,[get_links]=debug,kitsune_p2p::spawn::actor::space::rpc_multi_logic[]=trace"],
                &["RUST_LOG=warn"],
            )
            .await;
        let stream = out.output_stream();
...
async fn generate_data(
    ...
    let data = match data_stream {
        ...
        DataStream::EChat => {
            let (hdks, sign_jh) = GenerateHdk {
                keystore: keystore.clone(),
                dna_hash: dna_hash.clone(),
                genesis_settings: (0..num_agents).map(|_| GenesisBuilder::default()).collect(),
                zome_id: 0.into(),
    ...
    let ideal_length =
        mock_network::agent_info::ideal_target(num_agents + 1, DhtArc::full(0)).half_length();
    settings.dht_storage_arc_half_length(ideal_length);
    ...
    let gen_data = mock_network::GeneratedData::new(data.clone(), agent_info.clone());
    dbg!(s.elapsed());
...
async fn send_db_and_hashes(
    ...
    .for_each_concurrent(5, |(i, (remote, url))| {
        let gen_data = gen_data.clone();
        let hashes = hashes.clone();
        async move {
            let (mut tx, mut rx) = controller::connect::connect(url).await;
