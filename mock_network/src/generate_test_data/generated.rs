...
impl GeneratedData {
    pub fn new(
        ...
        let all_held_agents: HashMap<_, Vec<_>> = agent_to_info
            ...
            .map(|(agent, info)| {
                (
                    ...
                    agent_info
                        .iter()
                        .filter(|other| info.storage_arc.contains(other.storage_arc.center_loc()))
                        .cloned()
                        .collect(),
                )
            })
            .collect();
        let ops_by_loc: BTreeMap<DhtLocation, Vec<(DhtOpType, Arc<Element>, Arc<KitsuneOpHash>)>> =
            data.values()
        ...
        let agent_loc_time_idx = agent_to_info.iter().fold(
            BTreeMap::new(),
            |mut map: BTreeMap<_, Vec<_>>, (agent, info)| {
                if let Some((s, e)) = info.storage_arc.interval().to_bounds_grouped() {
                    if s <= e {
                        let iter = ops_by_loc.range(s..=e).flat_map(|(l, v)| {
                            v.iter()
                                .filter(|(op_type, el, _)| check_public(op_type, el.as_ref()))
                                .map(|(_, el, hash)| (*l, el.header().timestamp(), hash.clone()))
                        });
                        for (l, t, h) in iter {
                            map.entry((agent.clone(), l, t)).or_default().push(h);
                        }
                    } else {
                        let iter = ops_by_loc
                            .range(..=e)
                ...
                if let Some((s, e)) = info.storage_arc.interval().to_bounds_grouped() {
                    if s <= e {
                        let iter = ops_by_loc.range(s..=e).flat_map(|(_, v)| {
                            v.iter()
                                .filter(|(op_type, el, _)| check_public(op_type, el.as_ref()))
                                .map(|(op_type, el, hash)| (hash.clone(), (*op_type, el.clone())))
                        });
