...
impl GenerateHdk {
    pub async fn make(self) -> (Vec<ExtractHdk>, tokio::task::JoinHandle<()>) {
        ...
        let ro_state = Arc::new(ReadOnlyState {
            ...
            entry_index,
            time_step,
            sys_time_calls: AtomicUsize::new(0),
...
impl hdk::hdk::HdkT for ExtractHdk {
    ...
    fn query(
        ...
        Ok(filter.filter_elements(elements))
    }
    ...
    fn create(
        ...
        Ok(self.state.share_mut(|state| {
            ...
            let timestamp = (last_header.timestamp() + self.ro_state.time_step).unwrap();
            let create = Create {
    ...
    fn sys_time(
        ...
        let time = self.ro_state.start_time + (n as u32) * self.ro_state.time_step;
