...
impl GeneratedData {
    ...
    pub fn peer_query(&self, authority: &AgentPubKey, loc: DhtLocation) -> Vec<AgentInfoSigned> {
        ...
        peers.sort_unstable_by_key(|info| shortest_arc_distance(loc, info.storage_arc.start_loc()));
        peers.truncate(8);
        peers
    }
    ...
    pub fn peer_get(
        ...
        new_peers: &HashMap<Arc<KitsuneAgent>, AgentInfoSigned>,
    ) -> Option<AgentInfoSigned> {
        let info = self
            .agent_to_info
            .get(&AgentPubKey::from_kitsune(&agent))
            .or_else(|| new_peers.get(&agent))
            .cloned()?;
        self.agent_to_info
            .get(authority)
            .unwrap()
            .storage_arc
            .contains(info.storage_arc.start_loc())
            .then(|| info)
    }
}
